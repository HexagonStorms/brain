...
description: qBittorrent seeding saturated the WiFi uplink and buffered Jellyfin; real fix is an API-set upload cap. qBit 5.x renamed the conf rate-limit key.
metadata: 
  node_type: memory
  type: project
...
```
The `Referer` header is required or qBit returns 403 Forbidden. To change the cap, use this or the WebUI at `qbit.plaza.codes` (Jo has the password). Media stack reference: `~/Code/elowynn-media-server/CLAUDE.md`. See [[reference_hetzner_vps]] for wider stack context.
